...
class BillingController extends AbstractController
{
    ...
    /**
     * @Route("/ajax_billing_delete_phone", methods={"DELETE"}, name="ajax_billing_delete_phone")
     */
    public function ajaxBillingDeletePhone(Request $request)
    {
        $this->get('session')->save();
        if (!$request->isXmlHttpRequest()) {
            return new JsonResponse([
            ...
            $client = new Client();
            $billing_info = new Billing();
            $phoneDelete = $billing_info->deletePhoneNumberFromContract($client, $request->request->get('contractId'), $request->request->get('phoneNumber'));
            return new JsonResponse([
                'status' => 'OK',

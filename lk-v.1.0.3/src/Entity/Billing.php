...
class Billing
{
    ...
    public function deletePhoneNumberFromContract(Client $client, $contractId, $phoneNumber)
    {
        if (preg_match("/^78722/", $phoneNumber)) {
            $phoneNumber = preg_replace("/^78722/", "", $phoneNumber);
        }
        $response = $client->request(
            "POST",
            $this->api_baseUrl,
            [
                'body' => '{
                    "method": "deletePhoneNumberFromContract",
                    "user": {
                        "user": "' . $this->credentials_user .'",
                        "pswd": "' . $this->credentials_password . '"
                    },
